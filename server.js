...
// ── Confirmed live column names from /api/schema (March 2026) ─────────────────
// Assessor (uzyt-m557):
//   pin, year, class, nbhd, certified_bldg, certified_land, certified_tot
// Characteristics (x54s-btds):
//   pin, year, char_yrblt, char_bldg_sf, char_land_sf, char_fbath, char_hbath,
//   char_frpl, char_type_resd, char_ext_wall, char_apts, char_gar1_size,
//   char_bsmt, char_air
// Address (3723-97qp):
//   pin, year, prop_address_full, prop_address_city_name
...
app.get('/api/pin/:pin', async (req, res) => {
  ...
  try {
    ...
    // Normalize to standard field names the frontend expects
    ...
    const chars = charsRow ? {
      pin:              charsRow.pin,
      tax_year:         charsRow.year,
      year_built:       charsRow.char_yrblt,
      building_sqft:    charsRow.char_bldg_sf,
      land_sqft:        charsRow.char_land_sf,
      num_full_baths:   charsRow.char_fbath,
      num_half_baths:   charsRow.char_hbath,
      num_fireplaces:   charsRow.char_frpl,
      type_of_residence:charsRow.char_type_resd,
      ext_wall_material:charsRow.char_ext_wall,
      num_apartments:   charsRow.char_apts,
      garage_size:      charsRow.char_gar1_size,
      basement_type:    charsRow.char_bsmt,
      central_air:      charsRow.char_air
    } : null;

    const addr = addrRow ? {
      pin:              addrRow.pin,
      tax_year:         addrRow.year,
